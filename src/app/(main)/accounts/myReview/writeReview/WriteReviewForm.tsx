    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-8 normal-14 tablet:normal-18">
      ...
      <div>
        ...
        <div className="pt-2">
          ...
          />
          <span className="text-gray-400 text-right block">{content.length} / 100</span>
        </div>
      </div>
      ...
      <button
        ...
        className={`flex items-center justify-center medium-14 px-[31px] py-[8px] border bg-primary-800 rounded-[4px] text-white w-full ${
          isSubmitting ? 'opacity-50 cursor-not-allowed' : 'hover:bg-primary-700 cursor-pointer'
        }`}
      >
  ...
  return (
    <div>
      ...
      <div className="flex flex-row">
        {[1, 2, 3, 4, 5].map(star => (
          <button key={star} type="button" className="pt-2 cursor-pointer" onClick={() => setRating(star)}>
            <StarIcon
              className={
                star <= rating
